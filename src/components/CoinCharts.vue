<template>
  <div class="info-card">
    <div class="row">
      <div class="chart-panel col" ref="chartdivday" id="chartdivday"></div>
    </div>
...
export default {
  ...
  computed: {
    ...mapState(["tickers", "suggestionDay"]),
  },
  watch: {
    tickers: {
      immediate: true,
  ...
  methods: {
    ...
    async fetchChartData(isUpdate = false) {
      ...
      const response = await fetch(
        `${BASE_URL}?symbol=${this.symbol}&interval=${this.interval}&limit=100`
      );
      ...
      store.commit("UPDATE_DAY_SUGGEST", resData.suggestion);
    ...
    showChart() {
      this.chart = window.AmCharts.makeChart("chartdivday", {
        type: "stock",
        theme: "light",
